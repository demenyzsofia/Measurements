...
def my_boxplot_auc(dataset1, dataset2, directory, legend, iterator, language):
    ...
    data_auc = [dataset1.iloc[:, -1], dataset2.iloc[:, -1]]

    fig = plt.figure()
    ax = fig.add_subplot(111)
    bp = ax.boxplot(data_auc, widths=(0.5, 0.5), patch_artist=True)
    ...
    ax.set_xlabel(data['detectorgroup_box']['detector'], **font, size=fontsize)
    ax.set_ylabel("AUC", **font, size=fontsize)


    if iterator == 1 :
        ax.set_title(data['detectorgroup_box']['auc_linear'], **font, size=fontsize - 2)
        plt.savefig(directory + 'AUC_linear.png')
    if iterator == 2:
        ax.set_title(data['detectorgroup_box']['auc_ensemble'], **font, size=fontsize - 2)
        plt.savefig(directory + 'AUC_outlier_ensemble.png')
    if iterator == 3:
        ax.set_title(data['detectorgroup_box']['auc_probabilistic'], **font, size=fontsize - 2)
        plt.savefig(directory + 'AUC_probabilistic.png')


def my_boxplot3_auc(dataset1, dataset2, dataset3, directory, legend, language):
    data = read_from_json(language)
    data_auc = [dataset1.iloc[:, -1], dataset2.iloc[:, -1], dataset3.iloc[:, -1]]

    fig = plt.figure()
    ax = fig.add_subplot(111)
    bp = ax.boxplot(data_auc, widths=(0.5, 0.5, 0.5), patch_artist=True)
    ...
    ax.set_title(data['detectorgroup_box']['auc_proximity'], **font, size=fontsize - 2)
...
def detectorgroup_box(language):
    ...
    for group in L:
        ...
        while file_index < len(group):
            if iterator < 4:
                ...
                directory = OUTPUT_PNG + group[file_index].split('_')[0] + '/' + 'detectorgroup_box/'
                file_index = file_index + 1
